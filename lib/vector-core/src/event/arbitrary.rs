...
impl Arbitrary for Name {
    ...
    fn shrink(&self) -> Box<dyn Iterator<Item = Self>> {
        Box::new(self.inner.shrink().map(move |x| Name { inner: x }))
    }
}
...
impl Arbitrary for Value {
    ...
    fn shrink(&self) -> Box<dyn Iterator<Item = Self>> {
        match self {
            Value::Bytes(bytes) => {
                let bytes = bytes.clone().to_vec();
                Box::new(bytes.shrink().map(move |x| Value::Bytes(Bytes::from(x))))
            }
            Value::Integer(int) => Box::new(int.shrink().map(move |i| Value::Integer(i))),
            Value::Float(flt) => Box::new(flt.shrink().map(move |f| Value::Float(f))),
            Value::Boolean(bl) => Box::new(bl.shrink().map(move |b| Value::Boolean(b))),
            Value::Timestamp(ts) => single_shrinker(Value::Timestamp(ts.clone())),
            Value::Map(map) => Box::new(map.shrink().map(move |mp| Value::Map(mp))),
            Value::Array(arr) => Box::new(arr.shrink().map(move |ar| Value::Array(ar))),
            Value::Null => single_shrinker(Value::Null),
        }
    }
}
